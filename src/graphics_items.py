    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.GraphicsItemChange.ItemPositionHasChanged:
            if self.scene():
                ...
                # COORDINATE FIX: Call the model's update method which handles int conversion
                self.scene().parent().diagram.update_port_position(
                    self.port_model.component.instance_name,
                    self.port_model.name,
...
class ComponentItem(QGraphicsRectItem):
            ...
            for child in self.childItems():
                if isinstance(child, PortItem):
                    ...
                    diagram.update_port_position(
                        child.port_model.component.instance_name,
                        child.port_model.name,
    ...
    # ... (hover and paint methods unchanged)
    def hoverEnterEvent(self, event):
...
# ResizeHandle and ConnectionLineItem are unchanged.
